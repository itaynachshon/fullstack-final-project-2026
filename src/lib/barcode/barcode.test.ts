...
describe("classifyBarcode — valid GTIN inputs", () => {
  it.each([
    ...
    ["EAN-13 with check digit 0 (sum divisible by ten)", "7290000000060", "7290000000060"],
    ["valid UPC-A zero-pads to 13", "036000291452", "0036000291452"],
    ["UPC-A leading zero is preserved in the canonical form", "034000470693", "0034000470693"],
    ["11-digit input (UPC-A that lost its leading zero) pads to 13", "34000470693", "0034000470693"],
    ["9-digit input pads to 13", "123456784", "0000123456784"],
    ["GTIN-14 with indicator 0 strips to 13", "07290000066318", "7290000066318"],
    ["GTIN-14 form of a UPC-A resolves to the same canonical", "00034000470693", "0034000470693"],
    ["whitespace separators are stripped", " 7290 0000 6631 8 ", "7290000066318"],
...
describe("classifyBarcode — RCN / store-internal codes", () => {
  it.each([
    // [label, raw input, expected canonical]
    ["13-digit 2xx prefix (Israeli weighed goods)", "2000000000008", "2000000000008"],
    ["13-digit 02x prefix", "0212345678909", "0212345678909"],
    ["12-digit UPC starting with 2 pads into the 02x RCN range", "212345678909", "0212345678909"],
    ["13-digit 04x company-internal prefix", "0400000000008", "0400000000008"],
    ["all-zeros GS1 prefix (company-internal numbering)", "0000000123457", "0000000123457"],
...
describe("classifyBarcode — invalid inputs", () => {
  it.each([
    ...
    ["too short (7 digits — UPC-E must be expanded before this module)", "1234567", /length/i],
    ...
    ["GTIN-14 with non-zero indicator (case code, check digit valid)", "17290000066315", /indicator 1/i],
...
describe("normalizeBarcode — structural canonicalization only", () => {
  it.each([
    ...
    ["a wrong check digit still normalizes (validation is separate)", "7290000066319", "7290000066319"],
    ["UPC-A pads to 13", "034000470693", "0034000470693"],
    ["separators are stripped before padding", "0729-0000-0663-18", "7290000066318"],
    ["GTIN-14 with indicator 0 strips to 13", "07290000066318", "7290000066318"],
    ["GTIN-14 with non-zero indicator has no canonical form", "17290000066315", null],
...
describe("isValidGtin — strict GTIN lengths + GS1 mod-10", () => {
  ...
  it.each([
    ["a 9-digit string (not a GTIN length, even with a fine check digit)", "123456784"],

...
| Testing | **Vitest** today; React Testing Library + Playwright planned for the dedicated test wave | The assignment's sanctioned toolset. Unit/integration coverage (barcode domain, lookup chain, actions, scanner state machine, scan flow) runs on Vitest; component/E2E layers are covered in the test specification document (a later deliverable). |
...
| Vitest (React Testing Library & Playwright planned for the test wave) | Dev/test | Assignment-sanctioned test tooling | Dev-time only; unit/integration coverage is Vitest today |
